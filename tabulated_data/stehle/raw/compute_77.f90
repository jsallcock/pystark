...
!==> cds (presentation type base de donnees)
...
!***    Temporary Files
character*20 tempfile           ! Added CDS, template filename
character*40 PATH2, afile        ! Added CDS, temporary file name
...
3000   format(1X, E10.3, 1x, e10.3, 2x, e10.3, 3x, e10.3, 2x, e10.3, &
        3x, e10.3, 2x, e10.3, 2x, e10.3, 2x, e10.3)
...
read(5, *) TEMP ! temperature in K
read(5, *) DENS ! electronic density in cm-3
tempfile = 'fort.'      ! Added CDS
read(5, '(A20)', end = 88) tempfile ! Added CDS
88     continue        ! Added CDS
...
do 444 id = 1, id_max ! loop over the density
    ...
    do 12 i = 1, itot
        read(16, 3000) din(1, i), sprof(1, i), sprofs(1, i), &
                sprof(2, i), sprofs(2, i), sprof(3, i), sprofs(3, i)
    12      continue
    ...
    do 15 i = 1, itot
        read(16, 3000) din(4, i), sprof(4, i), sprofs(4, i), &
                sprof(5, i), sprofs(5, i), sprof(6, i), sprofs(6, i)
    15     continue
    ...
    do 48 i = 1, itot
        read(16, 3000) din(7, i), sprof(7, i), sprofs(7, i), &
                sprof(8, i), sprofs(8, i), sprof(9, i), sprofs(9, i)
    48     continue
...
if (.FALSE.) then
    do 28 id = 1, id_max
        do 29 it = 1, 10
            if(Pr0(id, it).gt.1) go to 29
            do 30 i = 1, jtot(id, it)
                write(76, 7600) IN_cds, INP_cds, AMBDA, fainu, &
                        dense(id), F00(id), Tempe(it), &
                        Pr0(id, it), d1om(id, it, i), o1line(id, it, i), &
                        o1lines(id, it, i)
            30           continue
        29         continue
    28      continue
...
do 35 i = 2, npik
    dif = (dom0(i) - dom0(i - 1))
    if(dif.le.1.e-6) go to 35
    if((dif / dabs(dom0(i))).le.0.1) go to 35
    inc = inc + 1
    domm(inc) = dom0(i)
35      continue
jdom = inc
...
afile = PATH2(tempfile, '71')           ! Added CDS
!       !open(71, file=afile, status='new')     ! Added CDS
open(71, file = afile)                    ! Added CDS
...
do 110 id = 1, id_max
    do 111 j = 1, 10
        ...
        if(jtot(id, j).eq.0) go to 111  ! correction dec 2008
        do 112 i = 2, jdom
            domeg = domm(i)
            ij_max = jtot(id, j)
            do  36 ij = 2, ij_max - 1
            ...
            test = &
                    (domeg - dom(id, j, ij_max - 1)) * (domeg - dom(id, j, ij_max))
            if(test.le.0.) then
...
if(TEMP.le.Tempe(1))  then
    write(6, *) ' your input temperature'
    write(6, *)&
            ' is smaller than the mimimum temperature tabulated'
    write(6, *) ' for this transition, which is'
...
if(pr01.gt.1.) stop
!***********************************************************
afile = PATH2(tempfile, '72')           ! Added CDS
!       !open(72, file=afile, status='new')     ! Added CDS
open(72, file = afile)                    ! Added CDS
afile = PATH2(tempfile, '74')           ! Added CDS
!       !open(74, file=afile, status='new')     ! Added CDS
open(74, file = afile)                    ! Added CDS
afile = PATH2(tempfile, '81')           ! Added CDS
!       !open(81, file=afile, status='new')     ! Added CDS
open(81, file = afile)                    ! Added CDS
!***********************************************************
...
do 41 id = id1, id2
    ...
    do 42 i = 1, jdom
        uprof(id, i) = tprof(id, it1, i) + (TEMP - tempe1) * &
                (tprof(id, it2, i) - tprof(id, it1, i)) / (tempe2 - tempe1)
        uprofs(id, i) = tprofs(id, it1, i) + (TEMP - tempe1) * &
                (tprofs(id, it2, i) - tprofs(id, it1, i)) / (tempe2 - tempe1)
        !               write(72,9100)
...
write(6, *)&
        '                 I(delta_nu)=I(-delta_nu)'
write(6, *)

write(6, *)&
        ' first column is the corresponding detuning in Angstroms'
write(6, *)&
        ' second column is the detuning delta_nu'
write(6, *)&
        ' column 3: Doppler convolved profile in s'
write(6, *)&
        '                 I(delta_nu)=I(-delta_nu)'
...
do 43 i = 1, jdom
    wprof = uprof(id1, i) + (DENS - dense1) * &
            (uprof(id2, i) - uprof(id1, i)) / (dense2 - dense1)
    wprofs = uprofs(id1, i) + (DENS - dense1) * &
            (uprofs(id2, i) - uprofs(id1, i)) / (dense2 - dense1)
    write(72, 9100)&
            domm(i), wprof, wprofs, fainom
    !
    ! further conversions
...
DOUBLE PRECISION FUNCTION FINTRP(X1, X2, X3, Y1, Y2, Y3, X)
    ...
    !
    ! L'INTERPOLATION EST :
    ! SI LA FONCTION EST MONOTONE (CROISSANTE OU DECROISSANTE)
    !    HYPERBOLIQUE : Y=A+B/(X-C)
    !    OU LINEAIRE  : Y=A*X+B
    ! SI LA FONCTION N'EST PAS MONOTONE
    !    PARABOLIQUE  : Y=A*X*X+B*X+C
    !
    A12 = X1 - X2
    A22 = X1 - X3
    V1 = Y1 - Y2
...
SUBROUTINE PIKSRT(N, nut, ARR)
    !
    ! SORTING BY STRAIGHT INSERTION ; CF NUMERICAL RECIPES P 226
    !
    IMPLICIT DOUBLE PRECISION (A-H, O-Z)
    DIMENSION ARR(Nut)
    DO 12 J = 2, N
...
END
!//////////////////////
CHARACTER*40 FUNCTION PATH2(d1, d2)
    !***    Added CDS, Concatenates two character strings without embedded blanks.
    CHARACTER*(*) d1, d2
    INTEGER I
    I = INDEX(d1, ' ') - 1
    IF(I.LT.0) I = LEN(d1)
